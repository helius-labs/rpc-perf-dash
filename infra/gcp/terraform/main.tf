...
locals {
  ...
  # Secrets the worker reads as env vars. Mirrors the AWS Secrets Manager
  # keys in infra/cdk/lib/worker-stack.ts so the same source-of-truth env
  # file can seed both clouds.
  secret_keys = toset([
