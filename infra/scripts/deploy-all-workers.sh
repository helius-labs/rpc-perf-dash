...
# Shared cloud-auth probes (check_aws / check_gcp / check_cf + hint_*).
...
# Run the post-deploy health check automatically once all tiers succeed.
# Enable with VERIFY=1 or the --verify flag.
VERIFY="${VERIFY:-0}"
for arg in "$@"; do
  [[ "$arg" == "--verify" ]] && VERIFY=1
done
...
# AWS profile with access to the account that holds your worker fleet and the
# canonical `rpcbench/env` secret. Used both to `cdk deploy` the AWS workers AND
# (via the shared-env rebuild) to read `rpcbench/env`. Set AWS_PROFILE to your
# own profile before running.
AWS_PROFILE_NAME="${AWS_PROFILE:?set AWS_PROFILE to your AWS profile}"
...
# Deploy config (PROJECT_ID, CLOUDFLARE_ACCOUNT_ID, …) lives in a gitignored
# .ops.env at the repo root, written by `pnpm bootstrap:creds`. Auto-load it so
# the GCP/CF tiers get their account ids without a manual `source`. Real
# environment values still win (only set what's currently unset).
OPS_ENV_FILE="$REPO_ROOT/.ops.env"
...
if [[ "$SKIP_AWS" != "1" ]]; then
  ...
  check_aws "$AWS_PROFILE_NAME" || fail "AWS session expired — $(hint_aws "$AWS_PROFILE_NAME")"
fi
...
if [[ "$SKIP_CF" != "1" ]]; then
  ...
  check_cf || fail "Cloudflare auth expired — $(hint_cf)"
fi
...
if [[ "$SKIP_GCP" != "1" ]]; then
  ...
  check_gcp || fail "gcloud auth expired — $(hint_gcp)"
fi

# ── Tier deploy functions ────────────────────────────────────────────────
# Each tier is a function so the four independent clouds can run CONCURRENTLY
# (see the parallel runner below). Within a tier:
#   - AWS stays SERIAL per region: cdk-assets shares one docker image tag across
#     regions and parallel `cdk deploy`s race on it (the header's docker-tag note).
#   - TeraSwitch hosts run in PARALLEL (independent boxes, independent ssh+rsync).
#   - CF and GCP are single script calls each.
# `fail` inside a tier exits only that tier's subshell; the runner aggregates.
...
deploy_tsw() {
  ...
  [[ ${#failed[@]} -eq 0 ]] || fail "deploy-tsw.sh failed for: ${failed[*]}"
  log "TSW done."
}
...
# GCP: build image, then terraform apply to all 6 regions. Assumes the one-time
# shared resources already exist (see infra/gcp/README.md for a fresh project).
deploy_gcp() {
  local GCP_IMAGE_URI
  ...
  # Terraform's google provider needs credentials. ADC is blocked by org policy
  # here, so mint a short-lived token from the existing 'gcloud auth login'
  # session via GOOGLE_OAUTH_ACCESS_TOKEN. Respect a caller-provided token.
...
# The four clouds are independent, so deploy them at once (was serial ≈ sum of
# all four ≈ 30 min → now ≈ the slowest single tier). Each tier streams to its
# own logfile; a live `tail -F` shows progress; failures are aggregated so one
# tier's failure doesn't abort the others. (They each build the worker docker
# image separately — the daemon runs the builds concurrently; BuildKit layer
# cache keeps the repeated builds cheap.)
LOGDIR="$(mktemp -d "${TMPDIR:-/tmp}/rpcbench-deploy.XXXXXX")"
BG_PIDS=(); BG_NAMES=()

launch() {  # launch <fn> <name>
  : > "$LOGDIR/$2.log"
  ( "$1" ) >"$LOGDIR/$2.log" 2>&1 &
  BG_PIDS+=("$!"); BG_NAMES+=("$2")
}

if [[ "$SKIP_AWS" != "1" ]]; then launch deploy_aws aws; fi
if [[ "$SKIP_TSW" != "1" ]]; then launch deploy_tsw tsw; fi
if [[ "$SKIP_CF"  != "1" ]]; then launch deploy_cf  cf;  fi
if [[ "$SKIP_GCP" != "1" ]]; then launch deploy_gcp gcp; fi

[[ ${#BG_PIDS[@]} -gt 0 ]] || fail "all tiers skipped — nothing to deploy."

log "deploying ${#BG_NAMES[@]} tier(s) IN PARALLEL: ${BG_NAMES[*]}"
log "live logs → tail -F $LOGDIR/*.log"

# Stream all tier logs live while they run; stop the tailer once tiers finish.
tail -n +1 -F "$LOGDIR"/*.log & TAIL_PID=$!

DEPLOY_FAILED=()
for i in "${!BG_PIDS[@]}"; do
  wait "${BG_PIDS[$i]}" || DEPLOY_FAILED+=("${BG_NAMES[$i]}")
done
kill "$TAIL_PID" 2>/dev/null || true
wait "$TAIL_PID" 2>/dev/null || true

if [[ ${#DEPLOY_FAILED[@]} -gt 0 ]]; then
  fail "tier(s) FAILED: ${DEPLOY_FAILED[*]} — inspect $LOGDIR/<tier>.log"
fi

log "ALL DONE (parallel)."
log "Logs kept at $LOGDIR"

if [[ "$VERIFY" == "1" ]]; then
  log "verifying fleet health (allowing ~2 min for samples to flow first)…"
  sleep 120
  ( cd "$REPO_ROOT" && pnpm verify:deploy ) \
    || fail "post-deploy verification FAILED — inspect the output above."
  log "verification passed."
else
  log "Run the verifier to confirm fleet health (or pass --verify next time):"
  log "  pnpm verify:deploy"
fi
