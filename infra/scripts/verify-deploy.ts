 ...
 *   - the consensus_log shows mostly clean outcomes (the no-consensus rate is
...
// (challenges + samples) and reports consensus decisions, correctness, and
...
// Every method is correctness-scored. Helper retained as `false` so the
// per-method block below reads cleanly without a structural rewrite.
...
async function main() {
  ...
  for (const m of ALL_METHODS) {
    ...
    if ((row.pct_no_cons ?? 0) >= 90 && row.logged >= 10) {
      ...
      pass(`${m}: ${row.challenges} challenges, ${row.logged} logged (${row.pct_no_cons ?? 0}% no-cons)`);
    }
  }
  void isLatencyOnly;
