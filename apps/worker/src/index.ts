...
// A single claim→fanout→insert cycle must finish within this budget; past it we
// treat the iteration as wedged (see watchdog.ts) so the loop can't freeze on a
// half-open DB socket. Generous vs the 5s/10s per-call fanout budgets + insert.
const ITERATION_TIMEOUT_MS = 30_000;
// If the loop fails continuously (every iteration timing out / erroring) for
// this long, the pool is likely poisoned — exit so the orchestrator restarts us
// with a fresh process + connection pool. A single success or idle poll resets
// the streak, so this never fires during normal operation or a generator outage.
const WEDGE_RESTART_MS = 180_000;
 ...
 * trace endpoint: `cdn-cgi/trace` returns a key=value text block where
...
async function main() {
  ...
  // Heartbeat is best-effort: a missed beat is retried next interval, and a
  // transient DB blip must not take the work loop down with it.
  ...
  // Timestamp the current uninterrupted failure streak began, or null when the
  // loop is healthy. Reset on any iteration that makes progress (sample OR idle
  // poll). Read by the self-heal watchdog below.
  let failingSince: number | null = null;

  // Self-heal watchdog: if the loop has been failing continuously past the
  // wedge threshold, exit so the orchestrator restarts us with a fresh pool.
  // The heartbeat runs on a separate interval and would otherwise keep beating
  // through a wedge, hiding it — this is what actually recovers a stuck worker.
  setInterval(() => {
    if (shouldSelfHeal(failingSince, Date.now(), WEDGE_RESTART_MS)) {
      const secs = Math.round((Date.now() - failingSince!) / 1000);
      console.error(`[worker] wedged: ${secs}s of continuous iteration failure — exiting for restart`);
      process.exit(1);
    }
  }, WATCHDOG_CHECK_MS);
  ...
  while (true) {
    try {
      // withTimeout turns a hung DB await into a rejection so the loop resumes
      // instead of freezing forever on a half-open socket.
      const did = await withTimeout(processOne(db), ITERATION_TIMEOUT_MS, "processOne");
      failingSince = null; // progress (sampled or idle) → healthy streak reset
      if (!did) {
