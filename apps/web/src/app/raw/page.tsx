...
interface ConsensusAuditRow {
  audited_at: Date | string;
  canonical_hash: Buffer | string | null;
...
export default async function RawPage({
  ...
  let qlog: QuorumLogRow | null = null;
  let audit: ConsensusAuditRow | null = null;
    ...
    if (challenge) {
      ...
      const cnRows = await db().execute(sql`
        SELECT worker_provider, region, egress_path, connection_mode,
               voters, decision, decision_reason, dissenters,
               auditor_verdict, decided_at
        FROM consensus_log
      ...
      const aRows = await db().execute(sql`
        SELECT audited_at, canonical_hash, matched, error
        FROM consensus_audit WHERE challenge_id = ${id}::uuid
      `);
      ...
      <Collapsible className="mt-3" title="Consensus log (per vantage × mode)" defaultOpen>
        {consensus.length > 0 ? (
          ...
          <p className={EMPTY_CLS}>
            No consensus-log entries. These are written selectively (disputed, no-consensus, or a 1%
            archive sample), so a healthy, non-sampled challenge produces no row.
          </p>
        )}
      </Collapsible>

      <Collapsible className="mt-3" title="Finality re-verification">
        {audit ? (
          <pre className={PRE_CLS}>
            {safeStringify({
              audited_at: toIso(audit.audited_at),
              matched: audit.matched,
              canonical_hash: bytesToHex(audit.canonical_hash),
              error: audit.error,
            })}
          </pre>
        ) : (
          <p className={EMPTY_CLS}>
            Not yet re-verified. The deferred audit job re-checks finalized challenges and writes its
            result here once the target slot has settled (~10 min after creation).
          </p>
        )}
      </Collapsible>
 ...
 * For unknown shapes we fall back to a pretty-printed JSON block.
 */
