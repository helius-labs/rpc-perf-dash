...
export interface OverallLeaderRow {
  ...
  /** Plain-language reason the provider is unscored (total 0), else null. */
  failing_reason: string | null;
}
...
/**
 * Plain-language text for a raw `failing_reason` code (the DB gate label). Kept
 * jargon-free and number-free for the leaderboard UI — the honeypot gate reads
 * as a deep-archive reliability problem, which is what it actually detects
 * (honeypots are all deeply-finalized historical reads). See
 * docs/methodology.md § "Who qualifies".
 */
const FAILING_REASON_TEXT: Record<string, string> = {
  insufficient_samples: "Not enough samples yet",
  reliability_below_threshold: "Too many failed requests",
  correctness_below_threshold: "Too many incorrect responses",
  honeypot_pass_below_threshold: "Unreliable on old (deep-archive) data",
};

export function humanizeFailingReason(reason: string | null | undefined): string | null {
  if (!reason) return null;
  return FAILING_REASON_TEXT[reason] ?? "Below quality thresholds";
}
...
export function buildOverallLeaderRows(
  ...
  regionWeights: Partial<RegionWeights> = DEFAULT_REGION_WEIGHTS,
): OverallLeaderRow[] {
  ...
  return BENCHMARKED_PROVIDERS.map((provider) => {
    ...
    // Per-geo raw win rate, recorded only for geos where the provider is
    // eligible (has a scored entry) — region-blended below with the SAME weights
    // + eligible-subset renormalization the score uses, so the displayed win% and
    // the rank move together instead of the old region-unweighted pooled rate.
    const winRateByGeo = new Map<GeoRegion, number>();
    // Gate-failure reasons across geos where the provider is ineligible — used
    // to explain a total-0 (unscored) row in plain language.
    ...
    for (const o of perGeo) {
      ...
      if (!sp && r.failing_reason) {
        reasonTally.set(r.failing_reason, (reasonTally.get(r.failing_reason) ?? 0) + 1);
      }
      ...
      if (sp && (r.n_challenges_with_winner ?? 0) > 0) {
        winRateByGeo.set(o.geo, (r.n_wins ?? 0) / r.n_challenges_with_winner);
      }
    }
    const blendedWinRate = blendRegionScalar(winRateByGeo, regionWeights) ?? 0;
    const dominantReason = [...reasonTally.entries()].sort((a, b) => b[1] - a[1])[0]?.[0];
    return {
      provider_id: provider.id,
      provider_name: provider.name,
      caveat_flags: flags,
      caveat_explanation: flags.length > 0 ? explainAntiGamingFlags(flags) : "",
      total: s?.total ?? 0,
      failing_reason: s != null ? null : humanizeFailingReason(dominantReason),
...
export function buildPresetLeaderRows(
  ...
  return BENCHMARKED_PROVIDERS.map((provider) => {
    ...
    // Per-(method, geo) raw win rate, recorded only for cells where the provider
    // is eligible. Blended below (region-blend per method → method-blend) with the
    // same weights the score uses, so the displayed win% tracks the rank instead
    // of the old region-/method-unweighted pooled rate.
    const winRateByMethodGeo = new Map<string, Map<GeoRegion, number>>();
    // Per-method gate-failure reasons for cells where this provider is NOT
    // eligible — used to explain WHY it's excluded (method → reason-code → count).
    const failReasonByMethod = new Map<string, Map<string, number>>();
    ...
    for (const cell of cube) {
      ...
      const eligibleHere = cell.eligible.some((x) => x.provider_id === provider.id);
      if (!eligibleHere && r.failing_reason) {
        let byReason = failReasonByMethod.get(cell.method);
        if (!byReason) {
          byReason = new Map();
          failReasonByMethod.set(cell.method, byReason);
        }
        byReason.set(r.failing_reason, (byReason.get(r.failing_reason) ?? 0) + 1);
      }
      if (eligibleHere && (r.n_challenges_with_winner ?? 0) > 0) {
        let geoMap = winRateByMethodGeo.get(cell.method);
        if (!geoMap) {
          geoMap = new Map();
          winRateByMethodGeo.set(cell.method, geoMap);
        }
        geoMap.set(cell.geo, (r.n_wins ?? 0) / r.n_challenges_with_winner);
      }
    }

    // Region-blend each method's per-geo win rate, then method-blend those.
    const methodWinRate = new Map<string, number>();
    for (const [method, geoMap] of winRateByMethodGeo) {
      const blended = blendRegionScalar(geoMap, regionWeights);
      if (blended != null) methodWinRate.set(method, blended);
    }
    const blendedWinRate = blendMethodScalar(methodWinRate, methodWeights) ?? 0;
    ...
    // Explain exclusion in plain language: dominant gate-failure reason across
    // the methods this provider is ineligible on. With a single method in view
    // the method name is redundant (the board already scopes to it), so it's
    // named only when >1 method contributes. Falls back to the coverage note.
    let exclusionReason: string | null = null;
    if (!coverageOk) {
      const reasonTally = new Map<string, number>();
      for (const byReason of failReasonByMethod.values()) {
        for (const [code, n] of byReason) {
          reasonTally.set(code, (reasonTally.get(code) ?? 0) + n);
        }
      }
      const dominant = [...reasonTally.entries()].sort((a, b) => b[1] - a[1])[0]?.[0];
      const human = humanizeFailingReason(dominant);
      const methods = [...failReasonByMethod.keys()];
      exclusionReason = human
        ? methods.length > 1
          ? `${human} — ${methods.join(", ")}`
          : human
...
export function buildMiniScoreRows(
  ...
  if (selectedGeo) {
    ...
    return buildSingleLeaderRows(outcome).rows.map((r) => ({
      ...
      total: r.total,
      failing_reason: r.total > 0 ? null : humanizeFailingReason(r.failing_reason),
    }));
  }
