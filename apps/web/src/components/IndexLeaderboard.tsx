...
import { slugForProviderId, websiteForProviderId } from "@rpcbench/shared/providers";
...
interface IndexRow {
  ...
  // `winSub` is the region's win sub-score (0–100): the provider's win rate in
  // that region as a % of the region leader's — the per-region input to the
  // blended W sub-score. Surfaced in the region tooltip so a balanced win
  // profile (strong across regions) vs a lopsided one is visible.
  regionBlend: Array<{ label: string; weight: number; score: number; winSub: number }>;
    ...
    <li key={p.id} className={"idx-li" + (p.coverageOk ? "" : " opacity-55")}>
      ...
      >
        ...
        {(() => {
          const scoreEl = (
            <span className="idx-score" style={tierColor ? { color: tierColor } : undefined}>
              ...
              {showScore && <i className="idx-score-unit">/100</i>}
            </span>
          );
        ...
        <span className="idx-actions">
          {(() => {
            const website = websiteForProviderId(p.id);
            ...
            {p.regionBlend.length > 0 && (() => {
              ...
              return (
                <span className="idx-ds">
                  ...
                  <span onClick={(e) => e.stopPropagation()}>
                    <FloatingTooltip title="Region weights & win strength" trigger={
                      ...
                      <div className="text-left font-normal normal-case tracking-normal leading-normal font-mono">
                        <div className="text-[11px] text-neutral-400 mb-1 px-1.5">
                          Region weight &amp; win strength in the overall blend
                        </div>
                        {/* Column header: region · weight · win rate vs the
                            region's leader (0–100) · that value. */}
                        <div className="flex items-center gap-2.5 px-1.5 pb-1 text-[9px] uppercase tracking-wide text-neutral-600">
                          <span className="w-[88px]">region</span>
                          <span className="w-[36px] text-right">weight</span>
                          <span className="flex-1">win rate vs region leader</span>
                          <span className="w-[24px] text-right" />
                        </div>
                        {[...p.regionBlend]
                          .sort((a, b) => b.weight - a.weight)
                          .map((r) => {
                            // Strongest regions (top-2 by composite score) are
                            // marked with a faint light highlight behind the
                            // whole row, so the text stays its normal colour.
                            const isTop = topSet.has(r.label);
                            const pct = Math.max(0, Math.min(100, r.winSub));
                            return (
                              <div
                                key={r.label}
                                className={
                                  "flex items-center gap-2.5 text-[11px] leading-snug rounded px-1.5 py-0.5 " +
                                  (isTop ? "bg-[color-mix(in_srgb,var(--text)_9%,transparent)]" : "")
                                }
                              >
                                <span className={"w-[88px] " + (isTop ? "text-neutral-200 font-medium" : "text-neutral-300")}>
                                  {r.label}
                                </span>
                                <span className="w-[36px] text-right tabular-nums text-neutral-400">
                                  {(r.weight * 100).toFixed(0)}%
                                </span>
                                <span
                                  className="flex-1 h-[9px] rounded-sm bg-neutral-800 overflow-hidden"
                                  aria-hidden="true"
                                >
                                  <span
                                    className={"block h-full rounded-sm " + (isTop ? "bg-neutral-200" : "bg-neutral-500")}
                                    style={{ width: `${pct}%` }}
                                  />
                                </span>
                                <span className="w-[24px] text-right tabular-nums text-neutral-400">
                                  {Math.round(pct)}
                                </span>
                              </div>
                            );
