    ...
    <section className="lb-index">
      <ol className="idx-list">
        {ordered.map((p, i) => {
          ...
          // Color coding by score tier (green / amber / red). The score number
          // is tinted, and a score-proportional gradient line sits below the row
          // (black → tier color, ending at the score position) so the gaps
          // between providers read at a glance.
          const tierColor = ranked ? scoreColor(p.score) : null;
          // #1's name + rank take the provider's brand color (overrides the
          // default accent). Falls back to the accent if no brand color.
                        ...
                        <tbody>
                          {OVERVIEW_METHODS.map((m) => (
                            <tr key={m}>
                              ...
                              {OVERVIEW_GEOS.map((g) => {
                                const cell = methodRegionLatency[p.id]?.[m]?.[g];
