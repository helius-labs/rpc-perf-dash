...
interface Props {
  ...
  /** When true, the plot area shows a skeleton while the control bar (filters +
   *  chart-display toggles) stays mounted. Lets the caller swap in a fresh data
   *  slice without unmounting the bar — keeps the Method dropdown open and every
   *  filter visible during the fetch. `series`/`scoreSeries` should be the last
   *  good slice so the bar's derived controls stay valid. */
  loading?: boolean;
...
export function LatencyChart({
  ...
  // Loading a fresh slice: keep the control bar (filters + chart-display
  // toggles) mounted and skeleton only the plot. Unmounting the bar here is what
  // used to close the Method dropdown and hide the RPC/metric/percentile/bin/
  // outliers filters on every not-yet-cached method switch.
  if (loading) {
    return (
