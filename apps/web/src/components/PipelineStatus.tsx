...
import { fetchProviderHealth, fetchConsensusHealth } from "@/lib/health";
...
export async function StatusHealthSections() {
  const [health, consensusRates, consensusHealth] = await Promise.all([
    fetchProviderHealth(),
    fetchConsensusRates(),
  ...
  return (
    <>
      ...
      <ConsensusIntegrity rates={consensusRates} />
    </>
  );
}
