...
// Leaf subpaths (not the barrel): apiParams is imported by the client
// PerfExplorer (for buildPageUrl), and the barrel pulls timing.ts → node:tls,
// which can't be bundled client-side.
import {
  GEO_REGIONS,
  type ConnectionMode,
  type GeoRegion,
  type Method,
} from "@rpcbench/shared/types";
