...
const FIXED_COLORS: Record<string, string> = {
  helius: "#ff9a3c",            // orange
  helius_gatekeeper: "#c2410c", // dark orange
...
// leaderboard. Keyed by provider id (helius is benchmarked as `helius_gatekeeper`).
const BRAND_COLORS: Record<string, string> = {
  quicknode: "#6CFF75",
  helius: "#E84125",
  helius_gatekeeper: "#E84125",
  alchemy: "#363FF9",
  triton: "#A12CFF",
};
...
// Brand logo marks, served from /public/logos. Helius is benchmarked under the
// `helius_gatekeeper` id, so both map to the same mark.
const LOGOS: Record<string, string> = {
  helius: "/logos/helius.svg",
  helius_gatekeeper: "/logos/helius.svg",
...
const ANIMATED_LOGOS: Record<string, string> = {
  ...
  helius_gatekeeper: "/logos/animated/helius.html",
...
export function animatedLogoFor(providerId: string): string | null {
  return ANIMATED_LOGOS[providerId] ?? null;
}
