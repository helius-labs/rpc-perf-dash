...
async function fetchPipelineStatusImpl(): Promise<PipelineStatus> {
  ...
  // ttl_min_s <= 0 means challenges are born already expired (generator clock
  // skew or a TTL misconfiguration): workers claim nothing while the generator
  // heartbeat and dispatch counters all look alive.
    ...
    const state: StageState =
      r.hb_age_s == null
        ...
        : r.sampled_5m > 0
          ? "ok"
          : r.claimed_5m > 0
            ? "warn" // claiming but not producing samples → wedged work loop
            : "down";
    return {
