 ...
 *
 * `no_consensus` is intentionally omitted: ambiguous samples are excluded from
 * the "calls failed" count (success_rate_calls denominator) so they never
 * appear in a failure breakdown — see the rollup scf predicate.
 */
...
export const FAILURE_LABELS: Record<string, FailureDescription> = {
  ...
  data_mismatch: {
    label: "Wrong data",
    hint: "Response didn't match the consensus answer, with no other detected cause.",
  },
};
