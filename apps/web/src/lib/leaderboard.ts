...
/**
 * Cache TTL for all server-side data fetchers. Rollups refresh on the
 * generator's 5-min tick, so 120s staleness is well within the data's grain
 * while keeping the `unstable_cache` entry alive across a traffic burst (so the
 * first visitor per combo warms it and the rest are cache hits, not a stampede
 * of cold rollup reads). NOTE: this const is exported and also backs the public
 * /api/leaderboard, /api/providers/[id], the OG leaderboard image, provider
 * pages, and the home overview — all move to 120s freshness together. That's
 * intentional (uniform benchmark data), and the API routes already front this
 * with their own CDN s-maxage.
 */
export const CACHE_TTL_S = 120;
...
async function fetchAggregatesForGeoImpl(opts: AggregateOpts): Promise<RowAgg[]> {
  // All windows read the leaderboard precompute (leaderboard_agg, grain '1h'/'1d'),
...
async function fetchAggregatesFromPrecompute(
  ...
  const failTable = sql.raw("leaderboard_failures");
  ...
  const rows = await db().execute(sql`
    WITH agg AS (
      SELECT
        ...
        (sum(success_num)::real      / NULLIF((sum(sample_count_total) - sum(nofault_excluded_count))::real, 0))::real AS success_rate,
...
export async function fetchRankedOverall(opts?: {
  ...
  return buildOverallLeaderRows(blended, perGeo, DEFAULT_REGION_WEIGHTS);
}
 ...
 *
 * Reads the leaderboard precompute (leaderboard_agg, grain '1h'/'1d') instead of raw
 ...
 *
 * Same scan + index as fetchMethodLatency (`leaderboard_agg_method_latency`
 * on grain, worker_provider, methodology_version, window_start) and the same
 ...
 * precompute (leaderboard_agg, grain '1h'/'1d') grouped by time bucket and scores EACH
...
async function fetchScoreSeriesImpl(opts: ScoreQuery): Promise<ScoreSeries[]> {
  ...
  const aggRows = (await db().execute(sql`
    SELECT
      ...
      (r.success_num::real     / NULLIF((r.sample_count_total - r.nofault_excluded_count)::real, 0))::real AS success_rate,
