...
export function parseShareParams(
  ...
  // Regions: `regions=` list, else legacy singular `region=<geo>` (overall →
  // preset default), else the preset's region subset.
  const regionsRaw = getParam(src, "regions");
  const legacyRegion = getParam(src, "region");
  ...
  } else if (legacyRegion && REGION_SET.has(legacyRegion)) {
    regions = [legacyRegion as GeoRegion];
  } else {
