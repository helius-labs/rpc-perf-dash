...
/**
 * True iff THIS session still holds the generator advisory lock. Run on the
 * pinned lock client (max:1) so `pg_backend_pid()` is the lock-holding backend;
 * if that session was terminated (evicted) and reconnected, the new backend
 * holds no lock → returns false. Used by the leader to self-exit on lock loss.
 */
export async function verifyLeadership(db: DbClient): Promise<boolean> {
  const r = await firstRow<{ held: boolean }>(
    db,
    sql.raw(`
      SELECT EXISTS (
        SELECT 1 FROM pg_locks
        WHERE locktype = 'advisory'
          AND classid = ${ADVISORY_LOCK_CLASSID}
          AND objid   = ${ADVISORY_LOCK_OBJID}
