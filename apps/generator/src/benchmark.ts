 ...
 * challenges (derived via the utility endpoint), run fanout+buildSampleRows
 * (which decides consensus locally in the runner), and print a ranked report.
 *
 *   pnpm benchmark
...
async function runOneChallenge(opts: {
  ...
  const referenceForRow = {
    response: null,
    hash: Buffer.alloc(0),
    tip_slot: observer.tipSlot(),
  };

  // Insert a challenge straight to `ready`. Direct SQL because the CLI is
  // single-process and doesn't go through the createReadyChallenge fan-out path
  // (no assignments — the CLI is its own vantage).
...
async function main() {
  ...
  if (args.json) {
    ...
    // Deployment-specific: only the operator knows where their dashboard
    // lives, so the link is emitted only when NEXT_PUBLIC_SITE_URL is set.
    const siteUrl = process.env.NEXT_PUBLIC_SITE_URL;
    if (siteUrl) {
      console.log(`View on web:   ${siteUrl.replace(/\/+$/, "")}/run/${runId}`);
    }
  }
