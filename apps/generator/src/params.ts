 ...
 * deriveChallenge) to the positional JSON-RPC params array. Shared by the
 * generator (index.ts) and the benchmark CLI (benchmark.ts) so the two never
 * drift. NOTE: every method needs an explicit branch — the final fallthrough is
 * the getSignaturesForAddress shape ([address, options]); an unbranched method
 * would emit [undefined, …].
 */
