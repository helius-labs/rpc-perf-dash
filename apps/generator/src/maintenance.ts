 ...
 * interval and would starve tail work; see docs/operations.md):
 *
 *   1. trimReferenceResponses — nulls challenges.reference_response once a
 *      challenge is >6h old. For honeypots that JSON (the pre-seeded known
 *      answer) is dead weight after the ~30s active window: only /raw renders
 *      the payload (guarded to show "trimmed" past 6h). The row and
 *      reference_hash are kept forever. Normal challenges carry no payload.
 *
 ...
 * the partial index challenges_ref_pending_idx (0001_initial.sql); without it the
 ...
 * the consensus logs), then prune accumulated eligibility snapshots by their
