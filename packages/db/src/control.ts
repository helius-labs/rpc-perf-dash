 ...
 *
 * The generator writes a challenge and its assignments in one step. Correctness
 * is decided per-sample in the worker via majority consensus across the
 * benchmarked panel.
 */
...
/**
 * Create a `ready` challenge and fan out one assignment row per active vantage.
 *
 * `reference` carries the generator's tip slot for freshness. For honeypots it
 * also carries the pre-seeded known answer (hash + response); the worker's
 * record.ts short-circuits the consensus path on is_honeypot and classifies
 * directly against it. Normal challenges carry no reference answer.
 */
