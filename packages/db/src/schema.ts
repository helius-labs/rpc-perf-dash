...
export const samples = pgTable(
  ...
  (t) => ({
    ...
    // (connection_mode-leading) were REMOVED in migration 0004. Both dated from
...
// pairwise_wins: head-to-head (A-vs-B) win counts per (grain, geo, provider
// pair, method, mode, mv, time-bucket). Providers stored as a canonical
// unordered pair (provider_a < provider_b); a_wins/b_wins are faster-latency
// (ties → earlier started_at) counts, n_contested = a_wins + b_wins. No
// worker_provider dimension — pairwise is inherently cross-infra. Written by
// insertPairwiseWins (0002_pairwise_wins.sql).
function pairwiseWinsColumns() {
  return {
    geo: text("geo").notNull(),
    provider_a: text("provider_a").notNull(),
    provider_b: text("provider_b").notNull(),
    method: text("method").notNull(),
    connection_mode: text("connection_mode").notNull(),
    methodology_version: smallint("methodology_version").notNull(),
    window_start: timestamp("window_start", { withTimezone: true }).notNull(),
    ...
    n_contested: integer("n_contested").notNull().default(0),
  };
}

export const pairwise_wins = pgTable(
  "pairwise_wins",
  { ...grainColumn(), ...pairwiseWinsColumns() },
  (t) => ({
    pk: primaryKey({
      columns: [
        t.grain,
    ...
    // Head-to-head read filters on the provider pair (not geo), then sums geos.
    read: index("pairwise_wins_read").on(
      t.grain,
      t.provider_a,
      t.provider_b,
      t.method,
