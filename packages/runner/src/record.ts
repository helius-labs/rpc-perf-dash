...
function buildRowsForMode(
  ...
  if (!input.is_honeypot && decided.consensus.kind === "consensus") {
    ...
    // Fix 1a: inject consensusSlot = the MAX context.slot over the MAJORITY voters
    // (the freshest agreeing read) into the reference shape, so the mutable-value
    // classifier (valueDivergenceVerdict) treats a divergent provider as "fresher"
    // only when it read strictly newer than even the freshest agreeing provider.
    // (The consensus reference is one representative member; its own slot isn't
    // necessarily the majority's freshest.)
    if (consensusReferenceShape !== null && typeof consensusReferenceShape === "object") {
      let maxMajoritySlot: bigint | null = null;
      for (const r of input.fanoutResults) {
        if (!majorityIds.has(r.provider_id)) continue;
        const s = decided.attempts.get(r.provider_id)?.response_slot;
        if (s != null && (maxMajoritySlot === null || s > maxMajoritySlot)) maxMajoritySlot = s;
      }
      if (maxMajoritySlot !== null) {
        consensusReferenceShape = {
          ...(consensusReferenceShape as Record<string, unknown>),
          slot: Number(maxMajoritySlot),
        };
      }
    }
  }

  // Fix 1b: freshness_lag is measured against the CONTEMPORANEOUS panel — the
  // median of the exec-time provider tips the worker captured this fanout — instead
  // of the stale generation-time reference tip (which every provider is ~20 slots
  // ahead of, flatlining the Freshness sub-score at 100). Falls back to the
  // reference tip when no panel tips were captured.
  const panelTips = [...input.provider_tip_slots.values()].sort((a, b) =>
    a < b ? -1 : a > b ? 1 : 0,
  );
  const panelMedianTip: bigint =
    panelTips.length > 0
      ? panelTips[Math.floor((panelTips.length - 1) / 2)]!
...
function decideProviderOutcome(
  ...
  //    EXCEPTION (Fix 3): an operator-side quota/rate-limit error (HTTP-200 JSON-RPC
  //    error body) is the benchmark account's own billing cap, not a provider data
  //    or availability defect → no-fault (`operational_error`, out of both C and R).
  if (d.attempt.outcome === "body_invalid" || d.attempt.outcome === "rpc_error") {
    if (d.attempt.outcome === "rpc_error" && isOperationalError(d.single.body)) {
      return { correctness: "ambiguous", exclusion_reason: "operational_error" };
    }
...
function classifyWithFreshness(args: {
  ...
  // Within classifyWithFreshness an `ambiguous` verdict can only come from the
  // mutable-value divergence path (valueDivergenceVerdict fresher branch) — the
  // genuine no-consensus case is stamped directly in decideProviderOutcome step 5,
  // never here. So it's the no-fault "provider read a fresher slot" exclusion.
  else if (correctness === "ambiguous") exclusion_reason = "freshness_ahead";
