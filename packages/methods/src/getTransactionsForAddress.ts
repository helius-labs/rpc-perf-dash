 ...
 * (~325KB payload measured at limit 25), not just latency-driven.
 *
 * Derivation caveat: this custom method is served by the utility endpoint only
 * because it currently points at a Helius endpoint. If the utility endpoint is
 * ever repointed to a provider that doesn't serve it, derivation for this
 * method fails and no challenges are produced for it.
 */
    ...
    shape = {
      ...
      txs: entries
        ...
        .map((e) => ({
          ...
          slot: e.slot,
          err: normalizeTxErr(e.meta?.err ?? null),
          fee: e.meta?.fee ?? null,
