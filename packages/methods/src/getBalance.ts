...
function projectImpl(response: GetBalanceResponse): CanonicalProjection {
  ...
  // Hash the VALUE only; slot rides in shape for the freshness check.
...
export const handlers: MethodHandlers<GetBalanceParams, GetBalanceResponse> = {
  ...
  classify(projection, reference, providerTipSlot, referenceTipSlot): Correctness {
    // Value matches consensus → correct (regardless of slot). Otherwise adjudicate
    // the divergence by each response's own context.slot vs the majority's freshest
    // read: fresher → excluded (no-fault), same slot → incorrect, older → stale.
