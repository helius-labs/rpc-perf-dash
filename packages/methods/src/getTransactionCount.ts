 ...
 * getBlockHeight it is compared by value tolerance (consensus) and
 * `valueVerdict` (correctness), wired in record.ts.
 *
 * ⚠️ The tolerances below are an INITIAL estimate. They MUST be tuned against
 * the measured inter-provider spread (see docs/methodology.md § Projection &
 * equivalence) before reading the correctness number — too tight and
