...
function projectImpl(response: GetTransactionResponse | null): CanonicalProjection {
  ...
  const shape = {
    ...
    // Guard null meta (some txs return meta:null) — mirror getBlock's per-tx
    // handling so projection is null-safe and hashes stably.
    meta: response.meta
      ? {
          err: normalizeTxErr(response.meta.err ?? null),
