 ...
 * require `epoch` equality AND `absoluteSlot` within tolerance (tight for
 * consensus, wide for the auditor's t=0→t+δ drift). Wired in record.ts.
 *
...
import { freshnessVerdict, slotFromShape, SLOT_TOLERANCE, SLOT_AUDITOR_TOLERANCE } from "./freshness.js";
...
/** Auditor match: same epoch AND absoluteSlot within the wide tolerance. */
export function epochInfoProjectionsMatchAuditor(a: CanonicalProjection, b: CanonicalProjection): boolean {
  return epochSlotMatch(a, b, SLOT_AUDITOR_TOLERANCE);
}
