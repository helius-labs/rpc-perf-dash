 ...
 * across the panel (Helius/Triton; Alchemy/QN/Flux; SF Public — the Solana
 * Foundation public RPC, since removed from the panel): >99% of
 ...
 * so the panel reaches consensus and cross-camp worker comparisons land in
 * the same set.
 *
 * Projection: SET of returned signatures (sorted), each with slot, err,
 * confirmationStatus. Drops blockTime, memo, response order.
 *
 * Consensus tolerance: tip-anchored Jaccard ≥ 0.8 with full-set fallback; see
 ...
 * existing ones away.
 *
 * Used by both the worker-side panel consensus decision and the per-sample
 * classify. Callers pass the two projection shapes; this function
