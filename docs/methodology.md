...
and landed as a PR — deliberately manual, on no fixed cadence. Its `asOf` stamp
is taken from the API's `window_end` (the rollup bucket the numbers actually
describe), **never** from the clock at generation time, so the date on the card
and the numbers on the card always refer to the same period. The rendered card
shows that date and links to the live board, so a snapshot that isn't refreshed
presents as an old result rather than a current one.
