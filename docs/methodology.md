...
- `getTransactionsForAddress` — three voters (Helius, Alchemy, Quicknode).
...
**`getTransactionsForAddress`'s panel changed twice in August 2026, and we'd
...
### Snapshot embeds
The send board is also published as a **static, dated snapshot**
(`/embed/sends-snapshot`) for pages that can't carry a widget whose order
changes on its own. Two facts make that necessary: the top targets are routinely
separated by a fraction of a slot, and the live board scores a single 1-day
rollup bucket (~1.4k samples/target), so day-to-day reordering is largely
sampling noise rather than a change in anyone's service.

The snapshot is a checked-in file regenerated by `pnpm snapshot:sends --write`
and landed as a PR — deliberately manual, on no fixed cadence. It scores the
**most recent completed daily bucket** (`/api/sends?complete=1`), not the live
board's newest one: the 1d rollup re-folds the last 30 days every tick, so the
newest bucket is always the current, still-filling day — correct for a live
board, but a partial day is not a result worth publishing. Its `asOf` stamp is
that bucket's `window_start`, the day the numbers cover — **never** the clock at
generation time, and never `window_end`, which is the exclusive bound and would
date every snapshot a day late. The rendered card shows that date and links to
the live board, so a snapshot that isn't refreshed presents as an old result
rather than a current one.

A snapshot is a point-in-time result and is not represented as live: anyone
comparing it against `/sends` or `/api/sends` is comparing a dated figure with a
current one, and the date is on the card to make that legible.
