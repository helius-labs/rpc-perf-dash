...
`samples` and `landing_tx_results` are daily-partitioned (`samples_YYYYMMDD`). (`samples_archived` was removed on 2026-08-31; see **Raw-response retention** below.) `apps/generator/src/partitions.ts` runs `ensurePartitions(db)` at startup and every 6h, creating partitions **`PARTITION_LEAD_DAYS` (4) ahead** so a partition is never created just-in-time at the midnight-UTC boundary (a create racing live inserts can take an `ACCESS EXCLUSIVE` lock and convoy every subsequent insert). Guards: the create runs under a short `lock_timeout` (`PARTITION_LOCK_TIMEOUT`) and logs+retries next tick instead of crashing; the sample INSERT path carries its own `statement_timeout`/`lock_timeout` (`insertSamples` in `packages/db/src/samples.ts`) so a stalled insert errors and retries rather than holding a lock; the storage watchdog alerts if no samples are written for 3 min while live.
...
**No migration was written for any of this, deliberately.** `0001_initial.sql` simply no longer creates `samples_archived`, `samples_lookup_idx`, or `samples_dash_idx`, so a fresh bootstrap is correct with nothing to undo. (`migrate.ts` tracks by FILENAME with no checksum, so editing the baseline is safe: a database that already applied `0001` skips it forever, and the edit only affects fresh installs. Verified 2026-08-31 by applying the baseline into a throwaway schema and asserting the resulting objects.) Dropping already-dead objects from an already-bootstrapped database is an **ops action, not a schema change** — and writing it as a migration would have forced a code-before-migration deploy order, inverting the rule in CLAUDE.md and leaving a footgun for the next deploy. One-time cleanup for a pre-existing database:
...
| `samples_archived` | **removed** | — | Dropped 2026-08-31, and no longer created by `0001`: 84% of the database, zero readers. See **Raw-response retention** above. |
| `eligibility` | **3d** | `maintenance.ts` (`ELIGIBILITY_RETENTION`) | Write-only audit trail — nothing SELECTs it, so it does not need the 31d dashboard window `challenges` has. Pruned 26.8M → 2.1M rows on 2026-08-31 (8125 MB → 418 MB). `DELETE` alone does not reclaim: it needed `VACUUM (FULL, ANALYZE)`, which took 96s and blocked only the rollup's own INSERT. |
