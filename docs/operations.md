...
Migration `0003` also drops `samples_lookup_idx` and `samples_dash_idx`, and `0001_initial.sql` no longer creates them or `samples_archived` — so a fresh bootstrap never makes what `0003` would immediately drop, and `0003` is a pure no-op there. `migrate.ts` tracks by FILENAME with no checksum, so editing the baseline is safe for existing databases (they skip `0001` forever) and only affects fresh installs; `0003` is what fixes the databases that already ran the old baseline. Verified 2026-08-31 by applying all three migrations into a throwaway schema.
...
| `samples_archived` | **removed** | — | Dropped 2026-08-31 (migration `0003`, and no longer created by `0001`): 84% of the database, zero readers. See **Raw-response retention** above. |
